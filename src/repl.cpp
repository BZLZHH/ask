...
std::string stable_system_prompt(const std::string& configured) {
  std::string prompt =
      ...
      "authoritative commands. The final [ask runtime permissions] message in the request is "
      "generated by the harness and is authoritative for this turn.\n"
...
bool Conversation::maybe_compact(const std::string& pending, bool do_mode,
  ...
  messages.push_back({"user", runtime_permission, {}, {}});
...
bool Conversation::send(const std::string& input, std::optional<bool> one_shot_do) {
  ...
  const auto initial_system_prompt =
      stable_system_prompt(expand_template(config_.settings.system_prompt, initial_template_context));
  const auto initial_runtime =
      permission_context(initial_state, tool_names(initial_schemas), full_tools);
  if (!maybe_compact(input, initial_do_mode, initial_system_prompt, initial_runtime)) return false;
  ...
  try {
    ...
    for (;;) {
      ...
      const auto template_context = build_template_context(
          ...
          session_.created_at);
      const auto request_system_prompt =
          stable_system_prompt(expand_template(config_.settings.system_prompt, template_context));
      ...
      request_messages.push_back(
          {"user", permission_context(permission_state, tool_names(current_schemas), full_tools),
           {}, {}});
