...
void apply_conference_type_defaults(Conference& conference) {
  ...
  conference.next_speaker_reason = "Moderatorfirst confirms the conference scope, agenda, and speaker assignments.";
}
...
void ConferenceEngine::generate_setup_with_moderator() {
  try {
    ...
    if (conference_.status == ConferenceStatus::preparing) {
      ...
      conference_.status = ConferenceStatus::awaiting_setup;
      conference_.setup.rationale = "Moderatorplan generation failed; preserving an editable default plan.";
      record("setup_error", "System", "", "Moderatorplan generation failed; default plan preserved for User review.", error.what());
      persist();
    }
  }
}
...
std::vector<Message> ConferenceEngine::prompt_messages(const ConferenceParticipant& participant,
  ...
  meeting << (agenda == conference_.agenda.end() ? "Unspecified" : agenda->title);
    ...
    if (active != conference_.agenda.end()) {
      ...
      if (agenda_action == "next" || agenda_action == "complete") {
        ...
        if (!agenda_conclusion.empty()) {
          record("agenda", "Moderator #0", "Moderator", "Moderatorupdated current agenda item phase conclusion.", agenda_conclusion);
        }
      }
      changed = true;
    }
...
void ConferenceEngine::advance_with_policy(bool allow_write,
  ...
  try {
    ...
    for (;;) {
      ...
      if (response.finish_reason == "length" || response.finish_reason == "max_tokens") {
        ...
        if (participant.kind == "moderator") {
          ...
          conference_.next_speaker_id = "moderator";
          conference_.next_speaker_reason = "advisor seatoutput was truncated by length limit; handed to Moderatorto evaluate existing portions and reassign.";
          conference_.return_to_moderator = false;
        }
        persist();
        return;
    ...
    record("interrupted", "System", "", "Current AI contribution interrupted by User interjection.", participant.name);
  } catch (const std::exception& error) {
    std::lock_guard lock(mutex_);
    if (active_stream_event_) mark_interrupted_event(*active_stream_event_, "requestfailed");
    active_stream_event_.reset();
    record("error", "System", "", "Member contribution failed: " + std::string(error.what()), participant.name);
...
void ConferenceEngine::mark_interrupted_event(std::size_t event_index, const std::string& reason) {
  ...
  event.detail = reason;
  if (event.content.empty()) event.content = " (contribution aborted)";
}
...
void ConferenceEngine::conclude() {
  ...
  record("system", "Moderator", "Moderator", "Conference is compiling candidate conclusions, objections, and action items.");
  for (auto& agenda : conference_.agenda) {
    if (agenda.status == "active" || agenda.status == "pending") agenda.status = "completed";
  }
  conference_.status = ConferenceStatus::completed;
  record("system", "System", "", "Conference completed; minutes can be viewed via Summary or exported.");
  persist();
}
...
std::string ConferenceEngine::build_summary() const {
  ...
  const auto write_capped_section = [&](const std::string& title,
    ...
    if (values.size() > shown) output << "\n ( additional " << (values.size() - shown) << " items not listed)";
  };
  ...
  if (conference_.agenda.empty()) output << " None";
